...
#     res = requests.get(f"https://statsapi.mlb.com/api/v1/people/{player.id}")
...
#             info = res['people'][0]  # Extract the first player's information

#             # Parse birth location
#             city = info.get('birthCity')
#             state = info.get('birthState')
#             country = info.get('birthCountry')
...
#             bats = info.get('batSide', {}).get('description', 'Unknown')
#             throws = info.get('pitchHand', {}).get('description', 'Unknown')
...
#                 #### Current Age: {info.get('currentAge', 'Unknown')}
#                 > **Height:** {height} | **Weight:** {weight} lbs | **Bats/Throws:** {bats}/{throws} | **Status:** {status}
#                 >
#                 > **Born:** {birth_location}
#                 '''
#             )
#         else:
#             st.error(f"No data found for player {player.name}.")
#     else:
#         st.error(f"Failed to fetch data for player {player.name} (HTTP {res.status_code}).")
        ...
        if 'people' in res and len(res['people']) > 0:
            ...
            if data and 'stats' in data:
                ...
                display_player_stats(
                    career_hitting, season_hitting, career_pitching, season_pitching
                )
            else:
                st.error(f"No statistics available for {player.name}.")
        else:
            st.error(f"No data found for player {player.name}.")
    else:
        st.error(f"Failed to fetch data for player {player.name} (HTTP {res.status_code}).")
...
def display_player_stats(career_hitting, season_hitting, career_pitching, season_pitching):
    st.subheader("Career and Season Statistics")
    
    # Career Hitting Stats
    if career_hitting:
        st.markdown("### Career Hitting")
        for stat, value in career_hitting.items():
            st.write(f"**{stat.capitalize()}:** {value}")

    # Season Hitting Stats
    if season_hitting:
        st.markdown("### Season Hitting")
        for stat, value in season_hitting.items():
            st.write(f"**{stat.capitalize()}:** {value}")

    # Career Pitching Stats
    if career_pitching:
        st.markdown("### Career Pitching")
        for stat, value in career_pitching.items():
            st.write(f"**{stat.capitalize()}:** {value}")

    # Season Pitching Stats
    if season_pitching:
        st.markdown("### Season Pitching")
        for stat, value in season_pitching.items():
            st.write(f"**{stat.capitalize()}:** {value}")
